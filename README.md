...
У каждой части своё окружение (`part_1`: uv; `part_2/task_*`: pip + `requirements.txt`).

## Что не в git

- `part_2/task_*/visualizations/` — полные PNG-наборы после прогона (коммитятся только `figures/` и несколько примеров)
- `part_2/task_2/outputs/` — mesh-файлы реконструкции
- `**/checkpoints/`, `*.pt`, `*.pth`
